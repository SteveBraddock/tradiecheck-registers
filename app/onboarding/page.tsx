...
function Btn({ children, onClick, variant='primary', disabled=false }) {
  const styles = {
    ...
    ghost: { background:'transparent', color:'#374151', border:'1px solid #D8E6EE' },
  }
...
export default function OnboardingPage() {
  ...
  const steps = [
    ...
    <div key={0}>
      ...
      <p style={{ color:'#374151', marginBottom:24 }}>Enter your email address to begin. We'll save your progress as you go.</p>
    ...
    <div key={1}>
      ...
      <p style={{ color:'#374151', marginBottom:24 }}>All three structures follow Path A  the same verification process.</p>
    ...
    <div key={2}>
      ...
      <p style={{ color:'#374151', marginBottom:24 }}>Enter your NZBN or Company Number. We'll verify directly with the Companies Register.</p>
      {!company ? (
        <>
          ...
          <div style={{ fontSize:12, color:'#374151', marginBottom:20 }}>Found at app.companiesoffice.govt.nz</div>
            ...
            <>
              ...
              {(company.directors||[]).filter(d=>d.roleStatus==='ACTIVE').map(d=>(
                <div key={d.fullName} onClick={()=>setDirector(d.fullName)} style={{ padding:'12px 16px', borderRadius:8, border:`2px solid ${director===d.fullName?'#1E90D4':'#D8E6EE'}`, background:director===d.fullName?'#E6F1FB':'#fff', cursor:'pointer', marginBottom:8, fontSize:14, fontWeight:500 }}>
                  {d.fullName} <span style={{ fontSize:12, color:'#374151' }}>. Director since {d.appointmentDate}</span>
                </div>
              ))}
            </>
          )}
    ...
    <div key={3}>
      ...
      <p style={{ color:'#374151', marginBottom:24 }}>We run an Equifax Standard Business Report. This is mandatory  you cannot opt out.</p>
      {!credit ? (
        <>
        ...
        <>
          <div style={{ textAlign:'center', padding:'28px 0', marginBottom:24 }}>
            <div style={{ fontSize:11, fontWeight:600, color:'#374151', textTransform:'uppercase', letterSpacing:'0.06em', marginBottom:10 }}>Equifax Business Score</div>
            ...
            <div style={{ fontSize:11, color:'#374151', marginBottom:16 }}>out of 1,200</div>
    ...
    <div key={4}>
      ...
      <p style={{ color:'#374151', marginBottom:24 }}>Upload your last three years of annual financial accounts. We calculate 11 ratios across 4 categories.</p>
      {ratioScore === null ? (
        <>
          ...
          {['Year 1 (most recent)','Year 2','Year 3'].map(y=>(
            <div key={y} style={{ border:`2px dashed ${insFile?'#6DBE45':'#D8E6EE'}`, borderRadius:8, padding:'18px', textAlign:'center', marginBottom:12, cursor:'pointer', background:'#F5F8FA' }} onClick={()=>setInsFile(true)}>
              <span style={{ fontSize:14, color:'#374151' }}>Annual accounts  {y} {insFile?'':''}</span>
            </div>
          ))}
        ...
        <>
          {[['Liquidity',Math.floor(ratioScore*0.31),20],['Leverage',Math.floor(ratioScore*0.23),15],['Profitability',Math.floor(ratioScore*0.23),15],['Efficiency',Math.floor(ratioScore*0.23),15]].map(([label,score,max])=>(
            <div key={label} style={{ marginBottom:14 }}>
              <div style={{ display:'flex', justifyContent:'space-between', fontSize:13, marginBottom:4 }}><span style={{ fontWeight:500 }}>{label}</span><span style={{ color:'#374151' }}>{score}/{max}</span></div>
    ...
    <div key={5}>
      ...
      <p style={{ color:'#374151', marginBottom:24 }}>Your combined credit and ratio score determines whether you can proceed.</p>
      <div style={{ textAlign:'center', padding:'32px', borderRadius:12, marginBottom:24, background:gatePass?'#EEF7E8':'#FEE2E2', border:`2px solid ${gatePass?'#6DBE45':'#DC2626'}` }}>
        <div style={{ fontSize:11, fontWeight:600, color:'#374151', textTransform:'uppercase', letterSpacing:'0.06em', marginBottom:10 }}>Combined Score</div>
        <div style={{ fontSize:72, fontWeight:800, lineHeight:1, color:gatePass?'#3E7A22':'#DC2626', marginBottom:10 }}>{combinedScore}</div>
        <div style={{ fontSize:13, color:'#374151', marginBottom:16 }}>out of 90 required</div>
      ...
      <div style={{ display:'flex', gap:16, marginBottom:24 }}>
        <div style={{ flex:1, padding:14, background:'#F5F8FA', borderRadius:8, textAlign:'center' }}>
          <div style={{ color:'#374151', fontSize:13, marginBottom:4 }}>Credit points</div>
          <div style={{ fontSize:24, fontWeight:700 }}>{credit?.result==='Auto-Pass'?25:15}/25</div>
          <div style={{ fontSize:12, color:'#374151' }}>{credit?.result}</div>
        </div>
        <div style={{ flex:1, padding:14, background:'#F5F8FA', borderRadius:8, textAlign:'center' }}>
          <div style={{ color:'#374151', fontSize:13, marginBottom:4 }}>Ratio points</div>
          <div style={{ fontSize:24, fontWeight:700 }}>{ratioScore}/65</div>
          <div style={{ fontSize:12, color:'#374151' }}>Required: {credit?.result==='Monitor'?55:45}</div>
        </div>
      </div>
    ...
    <div key={6}>
      ...
      <p style={{ color:'#374151', marginBottom:24 }}>Upload your Certificate of Currency. Minimum $2M public liability cover required.</p>
      <div onClick={()=>setInsFile(true)} style={{ border:`2px dashed ${insFile?'#6DBE45':'#D8E6EE'}`, borderRadius:8, padding:'24px', textAlign:'center', cursor:'pointer', background:insFile?'#EEF7E8':'#F5F8FA', marginBottom:20 }}>
        {insFile?<span style={{ color:'#3E7A22', fontWeight:600 }}> Certificate attached (mock)</span>:<span style={{ color:'#374151' }}>Click to attach Certificate of Currency</span>}
      </div>
    ...
    <div key={7}>
      ...
      <p style={{ color:'#374151', marginBottom:24 }}>Select all trade categories your business operates in.</p>
      {TRADE_GROUPS.map(g=>(
        <div key={g.group} style={{ marginBottom:20 }}>
          <div style={{ fontSize:11, fontWeight:700, color:'#374151', textTransform:'uppercase', letterSpacing:'0.06em', marginBottom:8 }}>{g.group}</div>
          {g.trades.map(t=>{
    ...
    <div key={8}>
      ...
      <p style={{ color:'#374151', marginBottom:24 }}>Upload 5 photos of your best work and provide 5 client email addresses. Minimum 3 of 5 survey responses required.</p>
      ...
      <div onClick={()=>setVideo(true)} style={{ border:`2px dashed ${video?'#6DBE45':'#D8E6EE'}`, borderRadius:8, padding:'18px', textAlign:'center', cursor:'pointer', background:video?'#EEF7E8':'#F5F8FA', marginBottom:24 }}>
        {video?<span style={{ color:'#3E7A22', fontWeight:600 }}> Video attached (mock)</span>:<span style={{ color:'#374151' }}>Add work video (recommended, under 3 mins)</span>}
      </div>
    ...
    <div key={9}>
      ...
      <p style={{ color:'#374151', marginBottom:24 }}>Automated scan across search engines, social media, news, and review platforms.</p>
    ...
    <div key={10}>
      ...
      <p style={{ color:'#374151', marginBottom:24 }}>All tiers include the TradieCheck Verified badge. Upgrade for TradieWallet and deeper monitoring.</p>
      {TIERS.map(t=>(
        <div key={t.id} onClick={()=>setTier(t.id)} style={{ padding:'20px 22px', borderRadius:10, border:`2px solid ${tier===t.id?'#1E90D4':t.highlight?'#E6F1FB':'#D8E6EE'}`, background:tier===t.id?'#E6F1FB':'#fff', cursor:'pointer', marginBottom:12, position:'relative' }}>
          ...
          <div style={{ display:'flex', justifyContent:'space-between', alignItems:'flex-start', marginBottom:10 }}>
            ...
            <div><span style={{ fontSize:24, fontWeight:800 }}>${t.price}</span><span style={{ fontSize:12, color:'#374151' }}>/mo</span></div>
          </div>
          <ul style={{ margin:0, padding:'0 0 0 16px', fontSize:13, color:'#374151', lineHeight:1.7 }}>{t.features.map(f=><li key={f}>{f}</li>)}</ul>
        </div>
      ))}
    ...
    <div key={11} style={{ textAlign:'center', padding:'20px 0' }}>
      ...
      <p style={{ fontSize:15, color:'#374151', lineHeight:1.7, marginBottom:28 }}>A Verification Analyst will review your documents within 3 business days. Two-person sign-off is required before your badge is issued.</p>
      ...
      <div style={{ fontFamily:"'Inter','Segoe UI','Helvetica Neue',Arial,sans-serif", minHeight:'100vh', background:'#F5F8FA', padding:'24px 16px 60px' }}>
        <div style={{ maxWidth:620, margin:'0 auto' }}>
          <div style={{ textAlign:'center', marginBottom:28 }}>
            ...
            <div style={{ fontSize:13, color:'#374151', marginTop:6 }}>Path A Verification  Company / Trust / Partnership</div>
          </div>

...
const PRODUCTS = [
  ...
  { id: "p_wallet",   from: "HO",  to: "IP",  label: "TradieWallet",      color: "#F0C040", dash: false, bi: false, phase: "1a",  desc: "Homeowner deposits job funds into escrow. Released at milestones. TradieCheck earns 1% (capped $1K) via split disbursement. Escrow provider costs ~$10/transaction." },
  { id: "p_wallet2",  from: "IP",  to: "T",   label: "Milestone Release", color: "#F0C040", dash: false, bi: false, phase: "1a",  desc: "99% of milestone funds released to tradie. 1% (capped $1K) to TradieCheck. Escrow provider handles split disbursement." },
  ...
  { id: "p_swallet",  from: "HC",  to: "IP",  label: "SubbieWallet",      color: "#E8A030", dash: false, bi: false, phase: "2",   desc: "HC ring-fences subcontract payments in escrow. 1% fee (capped $1K) minus ~$10 escrow provider fee. Only on standalone B2B jobs -- no double-count with TradieWallet." },
...
// Array to enforce correct display order
const PHASE_COLORS = [
  { key: "1a",  color: GREEN,     label: "Phase 1a -- Wellington launch" },
  { key: "1b",  color: "#3AAF6E", label: "Phase 1b -- National rollout" },
  { key: "2",   color: BLUE,      label: "Phase 2 -- B2B extensions" },
  { key: "3",   color: "#F0956A", label: "Phase 3 -- Development market" },
  { key: "TBD", color: "#64748B", label: "TBD -- Placeholder" },
];

function getPhase(key) { return PHASE_COLORS.find(p => p.key === key); }
...
function assignOffsets(products) {
  ...
  products.forEach(p => {
    ...
    offsets[p.id] = (idx - (group.length - 1) / 2) * 22;
  });
  return offsets;
}
...
export default function EcosystemDiagram() {
  ...
  const hasPhaseFilter = activePhases.length > 0;
  const isVisible      = (p) => !hasPhaseFilter || activePhases.includes(p.phase);
  const isHighlighted  = (p) => activeProduct === p.id;
  const isFaded        = (p) => (activeProduct && p.id !== activeProduct) || (hasPhaseFilter && !activePhases.includes(p.phase));
  ...
  return (
    <div style={{ fontFamily: FONT, background: DARK, minHeight: "100vh", color: "#C8DCF0", display: "flex", flexDirection: "column" }}>
      ...
      {/* Header */}
      <div style={{ padding: "14px 24px", borderBottom: "1px solid #D8E6EE", background: "#FFFFFF", display: "flex", alignItems: "center", gap: 20, flexWrap: "wrap" }}>
        <div style={{ display: "flex", alignItems: "center", gap: 16 }}>
          <img src="/TradieCheckLogo_transparent.png" alt="TradieCheck" style={{ width: 160, height: 48, objectFit: "contain" }} />
          <div style={{ borderLeft: "1px solid #D8E6EE", paddingLeft: 16 }}>
            <div style={{ fontSize: 11, color: "#7A9AB0", letterSpacing: 0.5, marginBottom: 2 }}>Internal Reference</div>
            <div style={{ fontSize: 17, fontWeight: 700, color: "#1A2A3A", letterSpacing: -0.3 }}>Product Ecosystem Map</div>
          </div>
        </div>

        <div style={{ marginLeft: "auto", display: "flex", gap: 8, flexWrap: "wrap", alignItems: "center" }}>
          {PHASE_COLORS.map(v => {
            const active = activePhases.includes(v.key);
            const dimmed = hasPhaseFilter && !active;
            return (
              <button key={v.key} onClick={() => togglePhase(v.key)} style={{
                background: active ? v.color : v.color + "CC",
          ...
          {(hasPhaseFilter || activeProduct) && (
            <button onClick={() => { setActivePhases([]); setActiveProduct(null); }} style={{
              background: "#E0E8EE", border: "1px solid #C0D0DA", color: "#4A6070",
      ...
      <div style={{ display: "flex", flex: 1 }}>
        <div style={{ flex: 1, position: "relative", overflow: "hidden" }}>
          <svg ref={svgRef} viewBox="0 0 1000 760" style={{ width: "100%", height: "100%", minHeight: 500 }}>
            ...
            {ACTORS.map(a => {
              ...
              const isPhaseActive = hasPhaseFilter && related.some(p => activePhases.includes(p.phase));
              return (
                <g key={a.id}>
        ...
        <div style={{ width: 280, background: MID, borderLeft: "1px solid #1A3050", padding: "20px 18px", overflowY: "auto", display: "flex", flexDirection: "column", gap: 16, fontFamily: FONT }}>
          {activeP ? (
            <>
              <div>
                <div style={{ fontSize: 9, letterSpacing: 2, textTransform: "uppercase", color: getPhase(activeP.phase)?.color || "#64748B", marginBottom: 4 }}>
                  {getPhase(activeP.phase)?.label}
                </div>
            ...
            <>
              ...
              <p style={{ fontSize: 13, color: "#4A7090", lineHeight: 1.7, margin: 0 }}>
                Select one or more <span style={{ color: "#9BB5CC" }}>phase filters</span> to isolate products by launch phase. Combine phases to see how they build together.
              </p>

...
export default function ActionsLog() {
  ...
  async function seedConstitutionRules() {
    ...
    const newEntries = rules.map((r, i) => ({ id: `constitution-${Date.now()}-${i}`, num: maxNum + i + 1, ...r, createdAt: now, updatedAt: now }));
    const { error } = await supabase.from('action_entries').insert(newEntries.map(entryToDb));
  ...
  async function handleSubmit() {
    ...
    if (editId) {
      const { error } = await supabase.from('action_entries').update({ action: form.action, decision: form.decision, owner: form.owner, category: form.category, priority: form.priority, status: form.status, due_date: form.dueDate, meeting: form.meeting, notes: form.notes, linked_rules: form.linkedRules || [], updated_at: new Date().toISOString() }).eq('id', editId);
      if (error) console.error('Error updating:', error);
    } else {
      const nextNum = entries.length > 0 ? Math.max(...entries.map(e => e.num || 0)) + 1 : 1;
      const now = new Date().toISOString();
      const { error } = await supabase.from('action_entries').insert(entryToDb({ id: Date.now().toString(), num: nextNum, ...form, createdAt: now, updatedAt: now }));
  ...
  async function handleStatusChange(id, status) {
    const { error } = await supabase.from('action_entries').update({ status, updated_at: new Date().toISOString() }).eq('id', id);
    if (error) console.error('Error updating status:', error);
    await loadEntries();
  }

  async function handleDelete(id) {
    const { error } = await supabase.from('action_entries').delete().eq('id', id);
  ...
  function exportCSV() {
    ...
    const rows = entries.map(e => CSV_COLUMNS.map(col => { if (col === "linkedRules") return escapeCsv((e.linkedRules || []).join(";")); return escapeCsv(e[col]); }).join(","));
  ...
  const CSV_HEADER_MAP = { "ref": "num", "action": "action", "decision": "decision", "status": "status", "priority": "priority", "owner": "owner", "category": "category", "due date": "dueDate", "duedate": "dueDate", "meeting / source": "meeting", "meeting": "meeting", "notes": "notes", "linked constitution rules": "linkedRules", "linkedrules": "linkedRules", "created": "createdAt", "createdat": "createdAt", "updated": "updatedAt", "updatedat": "updatedAt", "id": "id", "num": "num" };
        ...
        for (let i = 1; i < lines.length; i++) {
          ...
          headers.forEach((h, idx) => {
            ...
            if ((key === "createdAt" || key === "updatedAt" || key === "dueDate") && val) {
              ...
              if (parts) { if (key === "dueDate") val = `${parts[3]}-${parts[2].padStart(2,"0")}-${parts[1].padStart(2,"0")}`; else val = new Date(parts[3], parts[2] - 1, parts[1]).toISOString(); }
            }
            obj[key] = val;
          });
    ...
    <div style={{ fontFamily: "'Inter','Segoe UI','Helvetica Neue',Arial,sans-serif", minHeight: "100vh", background: TC_LIGHT_BG, color: TC_CHARCOAL }}>
      ...
      {importPreview && (
        <div style={{ position: "fixed", inset: 0, background: "rgba(0,0,0,0.5)", zIndex: 1000, display: "flex", alignItems: "center", justifyContent: "center" }}>
          <div style={{ background: "#FFF", borderRadius: 12, padding: 28, maxWidth: 560, width: "90%", boxShadow: "0 8px 32px rgba(0,0,0,0.2)" }}>
            ...
            <div style={{ fontSize: 13, color: "#666", marginBottom: 16 }}><strong>{importPreview.filename}</strong> — {importPreview.rows.length} action{importPreview.rows.length !== 1 ? "s" : ""} found</div>
            <div style={{ background: TC_LIGHT_BG, border: `1px solid ${TC_BORDER}`, borderRadius: 8, padding: 14, marginBottom: 20, maxHeight: 200, overflowY: "auto" }}>
              {importPreview.rows.slice(0, 8).map((r, i) => (
                <div key={i} style={{ fontSize: 12, padding: "4px 0", borderBottom: i < Math.min(importPreview.rows.length, 8) - 1 ? `1px solid ${TC_BORDER}` : "none", color: TC_CHARCOAL }}>
              ...
              {importPreview.rows.length > 8 && <div style={{ fontSize: 11, color: "#888", padding: "6px 0 0" }}>…and {importPreview.rows.length - 8} more</div>}
            </div>
            <div style={{ fontSize: 13, fontWeight: 700, color: TC_CHARCOAL, marginBottom: 10 }}>How would you like to import?</div>
            <div style={{ display: "flex", gap: 10, marginBottom: 20 }}>
              <button onClick={() => confirmImport("replace")} style={{ flex: 1, background: "#FDE8E8", color: "#B02020", border: "2px solid #F5A5A5", borderRadius: 8, padding: "12px 16px", fontWeight: 700, cursor: "pointer", fontSize: 13 }}>🔄 Replace All<br /><span style={{ fontWeight: 400, fontSize: 11 }}>Wipe existing, load CSV only</span></button>
              <button onClick={() => confirmImport("merge")} style={{ flex: 1, background: "#EDF6FC", color: "#2A7DAF", border: `2px solid ${TC_BLUE}`, borderRadius: 8, padding: "12px 16px", fontWeight: 700, cursor: "pointer", fontSize: 13 }}>🔀 Merge<br /><span style={{ fontWeight: 400, fontSize: 11 }}>Add new, update matching IDs</span></button>
            </div>
            <button onClick={() => { setImportPreview(null); setImportMode(null); }} style={{ width: "100%", background: "transparent", color: "#666", border: `1.5px solid ${TC_BORDER}`, borderRadius: 6, padding: "10px 16px", cursor: "pointer", fontSize: 13 }}>Cancel</button>
          </div>
        </div>
      )}
      ...
      <div style={{ maxWidth: 1000, margin: "0 auto", padding: "24px" }}>
        ...
        {showForm && (
          <div style={{ background: "#FFF", border: `1.5px solid ${TC_BLUE}`, borderRadius: 10, padding: 24, marginBottom: 24 }}>
            ...
            <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr 1fr", gap: 12, marginBottom: 12 }}>
              <div><label style={labelStyle}>Owner</label><select value={form.owner} onChange={e => setForm(f => ({ ...f, owner: e.target.value }))} style={selectStyle}>{OWNERS.map(o => <option key={o}>{o}</option>)}</select></div>
              <div><label style={labelStyle}>Category</label><select value={form.category} onChange={e => setForm(f => ({ ...f, category: e.target.value }))} style={selectStyle}>{CATEGORIES.map(c => <option key={c}>{c}</option>)}</select></div>
              <div><label style={labelStyle}>Priority</label><select value={form.priority} onChange={e => setForm(f => ({ ...f, priority: e.target.value }))} style={selectStyle}>{PRIORITIES.map(p => <option key={p}>{p}</option>)}</select></div>
            </div>
            <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr 1fr", gap: 12, marginBottom: 12 }}>
              <div><label style={labelStyle}>Status</label><select value={form.status} onChange={e => setForm(f => ({ ...f, status: e.target.value }))} style={selectStyle}>{STATUSES.map(s => <option key={s}>{s}</option>)}</select></div>
              <div><label style={labelStyle}>Due Date</label><input type="date" value={form.dueDate} onChange={e => setForm(f => ({ ...f, dueDate: e.target.value }))} style={inputStyle} /></div>
              <div><label style={labelStyle}>Meeting / Source</label><input type="text" value={form.meeting} onChange={e => setForm(f => ({ ...f, meeting: e.target.value }))} placeholder="e.g. Weekly Sync 27 Feb" style={inputStyle} /></div>
            </div>
              ...
              <div style={{ marginBottom: 16 }}>
                ...
                <div style={{ border: `1.5px solid ${TC_BORDER}`, borderRadius: 6, background: "#FAFCFE", padding: 10, maxHeight: 200, overflowY: "auto" }}>
                  {constitutionRules.map(rule => {
                    ...
                    return (
                      <div key={rule.id} onClick={() => { const current = form.linkedRules || []; setForm(f => ({ ...f, linkedRules: linked ? current.filter(id => id !== rule.id) : [...current, rule.id] })); }} style={{ display: "flex", alignItems: "flex-start", gap: 10, padding: "7px 8px", borderRadius: 5, cursor: "pointer", background: linked ? "#EDF6FC" : "transparent", marginBottom: 2, border: linked ? `1px solid ${TC_BLUE}` : "1px solid transparent" }}>
                        ...
                        <div>
                          <div style={{ fontSize: 12, fontWeight: 700, color: TC_CHARCOAL }}>{rule.action}</div>
                          <div style={{ fontSize: 11, color: "#888", marginTop: 1 }}>{(rule.decision || "").slice(0, 100)}{(rule.decision || "").length > 100 ? "…" : ""}</div>
                        </div>
                      </div>
                    );
                  })}
                </div>
                {(form.linkedRules || []).length > 0 && <div style={{ fontSize: 11, color: TC_BLUE, marginTop: 4 }}>{(form.linkedRules || []).length} rule{(form.linkedRules || []).length !== 1 ? "s" : ""} linked</div>}
              </div>
            )}
            <div style={{ display: "flex", gap: 10 }}>
              <button onClick={handleSubmit} style={{ background: TC_CHARCOAL, color: "#FFF", border: "none", borderRadius: 6, padding: "10px 20px", fontWeight: 700, cursor: "pointer", fontSize: 14 }}>{editId ? "Save Changes" : "Add Action"}</button>
              <button onClick={resetForm} style={{ background: "transparent", color: "#666", border: `1.5px solid ${TC_BORDER}`, borderRadius: 6, padding: "10px 16px", cursor: "pointer", fontSize: 14 }}>Cancel</button>
            </div>
          </div>
        )}
        ...
        <div style={{ display: "flex", gap: 10, marginBottom: 20, flexWrap: "wrap", alignItems: "center" }}>
          ...
          <div style={{ fontSize: 13, color: "#888", marginLeft: "auto" }}>{filtered.length} item{filtered.length !== 1 ? "s" : ""}</div>
        </div>
        ...
        {loading ? (
          ...
          <div style={{ textAlign: "center", padding: 60 }}>
            ...
            <div style={{ color: "#888", fontSize: 15 }}>{entries.length === 0 ? "No actions yet. Add your first one!" : "No results match your filters."}</div>
          </div>
        ) : (
              ...
              return (
                <div key={entry.id} style={{ background: "#FFF", border: `1.5px solid ${overdue ? "#F5A5A5" : TC_BORDER}`, borderRadius: 10, padding: "16px 20px", borderLeft: `4px solid ${overdue ? "#E84040" : sc.dot}` }}>
                  ...
                  <div style={{ marginBottom: 10 }}>
                    ...
                    {entry.decision && (
                      <div style={{ fontSize: 13, color: "#555", borderLeft: `3px solid ${TC_GREEN}`, background: "#F6FAF0", borderRadius: "0 4px 4px 0", padding: "6px 10px", marginBottom: 4 }}>
                    ...
                    {entry.notes && <div style={{ fontSize: 13, color: "#888", marginTop: 4 }}>{entry.notes}</div>}
                    {entry.linkedRules && entry.linkedRules.length > 0 && (() => {
                      ...
                      return (
                        <div style={{ marginTop: 10, background: "#F0F7FC", border: `1px solid #C8E0EE`, borderRadius: 6, padding: "8px 12px" }}>
                          <div style={{ fontSize: 10, fontWeight: 700, color: TC_BLUE, textTransform: "uppercase", letterSpacing: "0.08em", marginBottom: 6 }}>Constitution Requirements</div>
                          {linked.map(rule => (
                            <div key={rule.id} style={{ marginBottom: 6, paddingBottom: 6, borderBottom: `1px solid #D8EDFB` }}>
                              <div style={{ fontSize: 12, fontWeight: 700, color: TC_CHARCOAL }}>{rule.action}</div>
                              <div style={{ fontSize: 11, color: "#555", marginTop: 2 }}>{rule.decision}</div>
                              {rule.notes && <div style={{ fontSize: 10, color: "#888", fontStyle: "italic", marginTop: 2 }}>{rule.notes}</div>}
                            </div>
                          ))}
                        </div>
                      );
                  ...
                  <div style={{ display: "flex", flexWrap: "wrap", gap: 8, alignItems: "center" }}>
                    <select value={entry.status} onChange={e => handleStatusChange(entry.id, e.target.value)} style={{ background: sc.bg, color: sc.text, border: "none", borderRadius: 20, padding: "4px 10px", fontSize: 12, fontWeight: 700, cursor: "pointer" }}>
                      {STATUSES.map(s => <option key={s}>{s}</option>)}
                    </select>
                    <span style={{ background: pc.bg, color: pc.text, borderRadius: 20, padding: "4px 10px", fontSize: 12, fontWeight: 700 }}>{entry.priority}</span>
                    <span style={{ fontSize: 12, color: "#555", display: "flex", alignItems: "center", gap: 4 }}>👤 {entry.owner}</span>
                    {entry.dueDate && <span style={{ fontSize: 12, color: overdue ? "#E84040" : "#666", display: "flex", alignItems: "center", gap: 4, fontWeight: overdue ? 700 : 400 }}>📅 {formatDate(entry.dueDate)}</span>}
                    {entry.meeting && <span style={{ fontSize: 12, color: TC_BLUE, display: "flex", alignItems: "center", gap: 4 }}>📋 {entry.meeting}</span>}
                    <span style={{ fontSize: 11, color: "#888", background: "#EDF2F5", padding: "3px 8px", borderRadius: 4 }}>{entry.category}</span>
                    <div style={{ marginLeft: "auto", display: "flex", gap: 8, alignItems: "center" }}>
                      <button onClick={() => handleEdit(entry)} style={{ background: "transparent", border: `1px solid ${TC_BORDER}`, borderRadius: 5, padding: "4px 10px", cursor: "pointer", fontSize: 12, color: TC_CHARCOAL }}>Edit</button>
                      {confirmDeleteId === entry.id ? (
                        <>
                          <span style={{ fontSize: 12, color: "#E84040" }}>Sure?</span>
                          <button onClick={() => handleDelete(entry.id)} style={{ background: "#E84040", border: "none", borderRadius: 5, padding: "4px 10px", cursor: "pointer", fontSize: 12, color: "#FFF", fontWeight: 700 }}>Yes</button>
                          <button onClick={() => setConfirmDeleteId(null)} style={{ background: "transparent", border: `1px solid ${TC_BORDER}`, borderRadius: 5, padding: "4px 10px", cursor: "pointer", fontSize: 12, color: "#666" }}>No</button>
                        </>
                      ) : (
                        <button onClick={() => setConfirmDeleteId(entry.id)} style={{ background: "transparent", border: "1px solid #F5A5A5", borderRadius: 5, padding: "4px 10px", cursor: "pointer", fontSize: 12, color: "#E84040" }}>Delete</button>
                      )}
                    </div>
                  </div>
                </div>
